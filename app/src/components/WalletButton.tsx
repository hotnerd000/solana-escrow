...
export default function WalletButton() {
  ...
  const handleConnect = async () => {
    // Dynamically look for any wallet that exposes standard features (like Phantom)
    const standardWallet = wallets.find(
      (w) => w.adapter.name === 'Phantom' || w.readyState === 'Installed'
    );
    
    if (standardWallet) {
      select(standardWallet.adapter.name);
    } else {
      // Fallback if no extensions are running locally
      setVisible(true);
    }
  };
